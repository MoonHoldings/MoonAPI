...
export const login = async (email: string, password: string, ctx: ExpressContext) => {
  ...
  ctx.res.cookie('jid', utils.createRefreshToken(user), { httpOnly: true, secure: true, sameSite: 'none' })
  ctx.res.cookie('aid', utils.createAccessToken(user, '1d'), { httpOnly: true, secure: true, sameSite: 'none' })
  return user
}
