...
export const login = async (email: string, password: string, ctx: ExpressContext) => {
  ...
  ctx.res.cookie('jid', utils.createRefreshToken(user), { httpOnly: true, secure: true, sameSite: 'none', maxAge: 24 * 60 * 60 })
  ctx.res.cookie('aid', utils.createAccessToken(user, '1d'), { httpOnly: true, secure: true, sameSite: 'none', maxAge: 24 * 60 * 60 })
  return user
}
