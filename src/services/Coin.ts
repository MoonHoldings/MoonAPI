...
export const saveCoinData = async (coinData: CoinData, user: User, walletAddress?: string, isVerified?: boolean): Promise<Coin> => {
  const coin = new Coin()
  ...
  if (walletAddress) {
    ...
    coin.verified = isVerified ? isVerified : false
  }

  return await coin.save()
}

export const deleteCoinData = async (id: number, user: User): Promise<boolean> => {
  const coin = await Coin.findOne({ where: { id: id, user: { id: user.id } } })
  if (!coin) {
    throw new UserInputError('Coin not found')
  }
...
export const connectCoins = async (walletAddress: string, user: User, isVerified: boolean): Promise<boolean> => {
  try {
    ...
    for (const balance of balances) {
      ...
      if (matchingCoin && balance.balance > 0) {
        ...
        const isExisting = await checkExistingCoin(newCoin, user, walletAddress, isVerified)

        if (!isExisting) {
          saveCoinData(newCoin, user, walletAddress, isVerified)
        }
      }
    }
...
export const checkExistingCoin = async (coinData: CoinData, user: User, walletAddress: string, isVerified: boolean) => {
  ...
  if (!coin) {
    ...
    coin.user = user
    coin.walletAddress = walletAddress
    coin.verified = coin.verified ? true : isVerified
