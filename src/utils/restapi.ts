...
router.post('/api/refresh_token', async (req, res) => {
  ...
  res.cookie('aid', utils.createAccessToken(user, '1d'), { httpOnly: true, secure: true })
...
router.get('/api/reset_password_callback/:token', async (req, res) => {
  ...
  try {
    ...
    if (user) {
      res.cookie('jid', utils.createAccessToken(user, '5m'), { httpOnly: true, secure: true })
  ...
  try {
    ...
    if (userInfo.email) {
      ...
      if (user) {
        ...
        res.cookie('jid', createRefreshToken(user), { httpOnly: true, secure: true })
